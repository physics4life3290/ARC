...
function collect_solver_input(primary_input)
    ...
    if primary_input.dimension == 1
        ...
        split_choice = prompt_choice("What type of operator splitting would you like to user?", splitting_choices)
        
        if primary_input.solver == :FTCS || primary_input.solver == :LaxFriedrichs || primary_input.solver == :Richtmyer
            return SolverInput(cfl, t_final, split_choice, false, false, nothing, nothing, nothing)
        elseif primary_input.solver == :GodunovScheme
            ...
            elseif reconstruction == :Linear || reconstruction == :Parabolic || reconstruction == :Cubic
            ...
            return SolverInput(cfl, t_final, split_choice, flattening, steepening, limiter, reconstruction, riemann_solve)
        end
    end
end