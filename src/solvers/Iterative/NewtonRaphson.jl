...
function newton_raphson(f, df, x0; tol=1e-10, maxiter=5000)
    ...
    elseif f isa AbstractArray 
        for k in 1:maxiter
            ...
            fx = lagrange_interp(xdata, ydata, x, degree=3, side=:center)

            # Numerical derivative using small finite difference
            h = 1e-6
            fxph = lagrange_interp(xdata, ydata, x + h, degree=3, side=:center)
            fxmh = lagrange_interp(xdata, ydata, x - h, degree=3, side=:center)
            dfx = (fxph - fxmh) / (2*h)
